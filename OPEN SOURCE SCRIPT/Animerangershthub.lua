...
if currentPlaceId ~= allowedPlaceId then
    warn("Scriot doesnt support in thisgame, use this game Anime Rangers X (Place ID: " .. tostring(allowedPlaceId) .. ")")
    return
end
...
KeySystem.CreateKeyUI = function()
    ...
    if success then
        print("Stellar | Key hợp lệ, đang tải script...")
    ...
    Title.Text = "Stellar | Anime Rangers X"
    ...
    GetKeyButton.MouseButton1Click:Connect(function()
        ...
        StatusLabel.Text = "Discord link has copied into clipboard"
        StatusLabel.TextColor3 = Color3.fromRGB(100, 255, 100)
    end)
...
print("Stellar | Anime Rangers X đang khởi động, vui lòng đợi 10 giây...")
wait(10)
...
RangerSection:AddToggle("AutoJoinRangerToggle", {
    ...
    Callback = function(Value)
        ...
        if autoJoinRangerEnabled then
            ...
            for _, isSelected in pairs(selectedRangerMaps) do if isSelected then
            ...
            for _, isSelected in pairs(selectedActs) do if isSelected then
...
challengeTimeDelayInput = ChallengeSection:AddInput("ChallengeTimeDelayInput", {
    ...
    Callback = function(Value)
        ...
        if numValue and numValue >= 1 and numValue <= 30 then
            ...
            print("Giá trị delay không hợp lệ (1-30)")
            if challengeTimeDelayInput and challengeTimeDelayInput.Set then challengeTimeDelayInput:Set(tostring(
                challengeTimeDelay)) end
        end
    end
})
...
autoTPLobbyDelayInput = InGameSection:AddInput("AutoTPLobbyDelayInput", {
    ...
    Callback = function(Value)
        ...
        if numValue and numValue >= 1 and numValue <= 60 then
            ...
            if autoTPLobbyDelayInput and autoTPLobbyDelayInput.Set then autoTPLobbyDelayInput:Set(tostring(
                autoTPLobbyDelay)) end
        end
    end
})
    ...
    elseif customMappings[unitCount] then
        ...
        for displaySlot, actualSlot in pairs(customMappings[unitCount]) do
            if tempSlots[actualSlot] then
                ...
                tempSlots[actualSlot].Name
                print("🔄 Mapped (" ..
                unitCount .. "/6): Game Slot " .. actualSlot .. " → UI Slot " .. displaySlot .. " (" .. unitName .. ")")
            end
        end
    else
...
originalPrint("================================================================")
originalPrint("Stellar | Anime Rangers X - Logs đã được tắt để tối ưu hiệu suất")
...
local function createEmbed(rewards, gameInfo)
    ...
    local embed = {
        ...
        footer = {
            text = "Stellar | Anime Rangers X • " .. os.date("%x %X"),
            icon_url =
            ...
            if isPlayerInMap() then
                ...
                if not fpsBoostScriptLoaded then
                    local success, err = pcall(function()
                        ...
                        "https://raw.githubusercontent.com/Kiet010402/FPS-BOOST/refs/heads/main/FPSBOOTS.lua"))()
...
-- end
print("Stellar | Anime Rangers X đã được tải thành công!")

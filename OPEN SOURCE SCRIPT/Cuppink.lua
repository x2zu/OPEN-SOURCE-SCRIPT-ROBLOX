...
local ReelMode = "Legit" -- Bisa diubah ke "Blatant"
...
function startCatching()
    ...
    if reelBar and isAutoCatch then
        reelfinishRemote:FireServer(100, true) -- Auto Catch dengan Perfect
    end
end
...
function startCatchingNot() 
    local reelBar = player.PlayerGui:FindFirstChild("reel") and player.PlayerGui.reel:FindFirstChild("bar")
    if reelBar and isAutoCatch then
        reelfinishRemote:FireServer(100, false) -- Auto Catch tanpa Perfect
    end
end
...
local function startAutoReel()
    if ReelMode == "Legit" then
        ...
        task.wait(2)
        startCatching() -- Panggil startCatching() untuk Perfect Catch
    elseif ReelMode == "Blatant" then
        startCatchingNot() -- Panggil startCatchingNot() untuk non-Perfect Catch
    end
end
